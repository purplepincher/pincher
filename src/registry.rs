...
//! from the Pincher package registry.
//! 
//! Security:
//! - Uses Bearer token authentication
//! - Requires cryptographic signature verification before publishing
//! - Server-side prevents package squatting via author_key_hash verification
...
impl PincherRegistryClient {
    ...
    /// Register a new developer account with the registry
    pub fn register_developer(&self, username: &str, public_key: &str) -> Result<String> {
        ...
        println!("[REGISTRY] Developer '{}' registered successfully", username);
        Ok(token)
    }
    ...
    pub fn register_package(
        ...
        println!("[REGISTRY] Package '{}' registered (ID: {})", name, package_id);
        Ok(package_id)
    }

    /// Publish a .nail bundle to the registry with its signature
    pub fn publish_package(&self, nail_path: &Path) -> Result<()> {
        ...
        println!("[*] Publishing package '{}' to registry...", package_name);
        println!("    Bundle size: {} bytes", package_bytes.len());
        println!("    Signature: {}...", &sig_string.trim()[..std::cmp::min(32, sig_string.trim().len())]);

        // Build multipart form to separate payload from verification data
        ...
        if response.status().is_success() {
            println!("[SUCCESS] Package '{}' pushed to Pincher Registry", package_name);
            Ok(())
        } else {
    ...
    pub fn download_package(&self, package_name: &str, version: &str, output_path: &Path) -> Result<PathBuf> {
        ...
        println!("[REGISTRY] Downloaded '{}' v{} ({} bytes)", 
            package_name, version, bytes.len());
        Ok(output_file)
    }
}
...
mod tests {
    ...
    fn test_registry_client_creation() {
        let client = PincherRegistryClient::new("https://registry.pincher.dev", "test-token");
        assert_eq!(client.registry_url, "https://registry.pincher.dev");
    }
}
