...
| Git dependencies | 🔴 needs fix (partially resolved) | `silo-core` was confirmed unused and **removed** from `[workspace.dependencies]`. `ternary-types` remains a git dependency (`pincher-core/Cargo.toml:30`) and is actively used in `pincher-core` and `hybrid-bridge` — crates.io rejects packages depending on unpublished/git crates. See [`docs/prep-notes/git-dependency-resolution-options.md`](prep-notes/git-dependency-resolution-options.md) for verification and three resolution options; this one still needs a human decision. |
