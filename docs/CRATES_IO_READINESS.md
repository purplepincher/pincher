...
| Hardcoded secrets / dead code | ✅ resolved | Confirmed `pincher-core/src/daemon.rs`, `registry.rs`, and `updater.rs` were unreachable (not declared in `pincher-core/src/lib.rs`, not referenced via `#[path]`, and not used by any `[[bin]]` target) and removed them entirely, eliminating the hardcoded `SUPER_INSTANCE_SHARED_SECRET_KEY_FOR_NAIL_INTEGRITY` secret from the publish surface. |
